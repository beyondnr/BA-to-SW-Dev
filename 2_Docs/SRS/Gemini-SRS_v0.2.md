...
- **자동 급여/수당 계산 및 증빙 생성 (F-05, Story 1)**
  ...
  - 급여 리포트 및 분쟁 대응용 **PDF 증빙 문서** 생성
  - 각 증빙 문서에 고유 해시값을 부여하고 최소 1년간 보관

- **단순 변경 이력 기록 (Simple Audit Log, F-06)**
  - 주요 데이터 변경에 대한 기본 이력(수행자, 시각, 작업)을 기록
...
- 다국어 UI 지원 (영어, 중국어 등)
- AI 기반 스케줄 자동 추천 (LLM 연동 포함)
- SMS/카카오톡 외 이메일, 푸시 알림 등 **복수 채널 동시 사용**
- 급여 리포트 엑셀(Excel) 형식 내보내기 (PDF 우선)
...
- **인프라 및 데이터 위치 제약**
  - 모든 인프라는 **AWS(Amazon Web Services)** 클라우드 기반으로 구축한다.
  - 데이터 저장·처리는 **국내 리전(ap-northeast-2)** 에서만 수행되고, 해외 리전은 사용하지 않는다.
...
- **운영 의존성**
  ...
  - 클라우드 네이티브 또는 오픈소스 기반의 모니터링·알림 서비스가 사용 가능하다.

- **사용자 특성 가정**
  - 주요 사용자(사장님)는 복잡한 디지털 UI에 익숙하지 않다.
  - 온보딩과 주요 화면은 단계 수를 최소화하고, 큰 글씨·분명한 색 대비·간단한 선택 흐름으로 설계해야 한다.

#### 1.2.4 Technology Stack and Architecture Principles (기술 스택 및 아키텍처 원칙)

- **C-TEC-001 (프론트엔드)**: 모든 프론트엔드 서비스는 Vite 환경 기반의 **React.js** 프레임워크를 사용한다. UI 구현 효율성을 위해 MUI 또는 Ant Design 같은 UI 컴포넌트 라이브러리를 적극 활용한다.
- **C-TEC-002 (백엔드)**: 모든 백엔드 서비스는 JVM 기반(**Java 17 + Spring Boot 3.x**)을 사용한다.
- **C-TEC-003 (데이터베이스)**: 데이터베이스는 **MySQL 8.x** InnoDB 엔진을 사용하며, AWS RDS와 같은 관리형 서비스를 우선적으로 고려한다.
- **C-TEC-004 (아키텍처)**: MVP 단계의 빠른 개발과 안정적 운영을 위해, 기능간 분산(MSA)을 최소화하고 단일 백엔드 서버(**Monolithic Architecture**) 구조를 채택한다.
- **C-TEC-005 (인프라)**: AWS 관리형 서비스(예: RDS, S3, SNS, EventBridge)를 적극 활용하여 초기 인프라 관리 부담을 최소화한다.
...
| **Evidence (증빙)**    | 분쟁·감독 시 제출하는 PDF 문서 및 관련 로그                                                     |
| **Audit Log**          | 주요 엔터티 변경 기록(누가, 언제, 무엇을, 어떻게 바꿨는지)을 담은 로그                           |
...
| EXT-03 | 오브젝트 스토리지               | 생성된 PDF 증빙 및 감사 로그 파일을 저장하는 스토리지 (예: AWS S3)                    | 클라우드 스토리지 API        |
| EXT-04 | 모니터링 서비스                 | 성능·에러·큐 상태 등을 수집하는 모니터링 시스템 (예: AWS CloudWatch)                  | HTTPS API / 에이전트         |
| EXT-05 | 장애 관리 서비스                | SLA·성능 위반 발생 시 알림을 전달하는 장애 관리 시스템 (예: AWS SNS)                  | HTTPS API / Webhook          |
...
| REQ-FUNC-006 | 승인 대기 대시보드                           | 활성 직원의 ≥70%가 가용시간을 제출한 경우, 대시보드에 “검토→조정→공지” 3단계 카드가 표시되어야 한다.                                                              | Story 2 AC1, F-03                      | Must              | **Given** 활성 직원의 70% 이상이 유효한 가용시간을 제출했을 때, **When** 사장님이 대시보드를 열면, **Then** 성능 요구사항(REQ-NF-018)을 만족하며 3단계 구조의 승인 대기 카드가 렌더링되어야 한다.      | UI 테스트, 성능 테스트                      |
| REQ-FUNC-007 | 클릭 기반 스케줄 편집                         | 스케줄 화면에서 교대 시간을 클릭하여 나타나는 수정 창을 통해 조정하고, 임시 버전으로 저장할 수 있어야 한다.                                                        | Story 2 AC2, F-03, PRD §6.1 Schedule   | Should           | **Given** 임시 스케줄이 열려 있을 때, **When** 사장님이 특정 교대를 클릭하여 수정하고 저장하면, **Then** 활성 스케줄에는 영향을 주지 않고 신규 버전의 Draft가 저장·재조회 가능해야 한다.                | UI 테스트, 버전 관리 테스트                |
...
| REQ-FUNC-013 | PDF 증빙 생성                                 | 승인된 스케줄·급여 데이터를 기반으로 PDF 증빙 문서를 생성·다운로드 링크를 제공해야 한다.                                                                         | Story 1 AC3, F-05                       | Must              | **Given** 특정 주차의 급여가 승인되었을 때, **When** 사장님이 증빙 내보내기를 요청하면, **Then** 성능 NFR(REQ-NF-003)을 만족하는 범위에서 PDF 다운로드 링크가 제공되어야 한다.                          | 내보내기 단위 테스트, 성능 테스트           |
...
| REQ-FUNC-015 | 단순 감사 로그(Simple Audit Log)               | 스케줄, 규칙, 급여 엔터티에 대한 주요 변경 이벤트(생성/수정/삭제)를 로그에 남겨야 한다.                                                                        | F-06, PRD §6.1 AuditLog                 | Could            | **Given** 해당 엔터티가 변경될 때, **When** 트랜잭션이 커밋되면, **Then** 수행자, 시각, 작업 유형, 대상 엔터티 ID를 포함한 로그 엔트리가 작성되어야 한다.                                               | 로그 검증, 단위 테스트                      |
| REQ-FUNC-016 | 직원 스케줄 알림                               | 스케줄 승인 또는 변경 시 해당 직원들에게 설정된 단일 채널로 알림을 발송해야 한다. (비용 최적화를 위해 카카오 알림톡을 우선 사용하고, 실패 시 SMS를 사용한다.) | Story 2 AC2, F-03                       | Must              | **Given** 스케줄 버전이 승인되었을 때, **When** 영향을 받는 직원이 존재하면, **Then** 15초 이내에 알림 발송을 시도하고, 성공/실패 상태를 기록해야 한다.                                          | 게이트웨이 모킹 통합 테스트                 |
| REQ-FUNC-017 | 직원 리마인더                                  | 승인된 스케줄을 24시간 이상 확인하지 않은 직원에게 리마인더를 보내야 한다.                                                                                     | Story 2 AC3                             | Could            | **Given** 스케줄이 승인된 상태에서, **When** 24시간이 경과했을 때 일부 직원이 미확인 상태라면, **Then** 해당 직원에게 재알림을 보내야 한다.                                                         | 배치/스케줄러 테스트, 분석 검증              |
...
| REQ-FUNC-022 | 엑셀(Excel) 증빙 내보내기                       | 급여 리포트 등 데이터를 엑셀 파일 형식으로 내보낼 수 있어야 한다.                                                                                             | F-05                                    | Could            | **Given** 급여 리포트가 생성되었을 때, **When** 사장님이 엑셀 내보내기를 요청하면, **Then** 엑셀 파일 다운로드 링크가 제공되어야 한다.                                                              | UI 테스트, 내보내기 테스트                  |
...
| REQ-NF-012  | 비용            | 매장당 월 운영 비용                                                                              | 활성 매장 1곳당 평균 월 운영 비용이 3,000원 이하를 유지해야 한다. (클라우드 스토리지 수명 주기 정책 등을 활용하여 비용을 최적화한다.) | 비용 모니터링, 재무 리포트               |
| REQ-NF-013  | 모니터링        | 실시간 모니터링 범위                                                                             | 시스템의 주요 메트릭(API 응답 시간, 에러율, 큐 상태 등)이 실시간에 가깝게 모니터링 대시보드에 표시되어야 한다.     | 대시보드 점검, 모니터링 설정 테스트      |
| REQ-NF-014  | 모니터링        | 알림 임계값                                                                                      | API p95 응답 시간이 1.2초를 초과하는 상태가 5분 이상 지속될 경우, 운영팀에게 장애 관리 채널을 통해 알림이 발생해야 한다. | 알림 시뮬레이션, 장애 대응 훈련          |
...
| REQ-NF-018  | 성능            | 사장님 대시보드 로딩 시간                                                                        | 사장님 대시보드의 승인 대기 카드 p95 로딩 시간이 2초 이하여야 한다.                                               | 부하 테스트, APM 메트릭                  |
...
|-----------|-----------------------------------------------|------------------------------------------------------------------|----------------------------------------------------|
| ST-Story-1 | 준법 급여 운영 (Story 1 — Lawful Payroll)    | REQ-FUNC-004, 005, 009–014, 018–020, 022; REQ-NF-001–007        | TC-001(가용시간 제출), TC-010(위반 차단), TC-020(급여 계산), TC-030(증빙 해시) |
| ST-Story-2 | 1클릭 스케줄 승인 (Story 2 — 1-click Approve)| REQ-FUNC-001, 006–008, 016, 017; REQ-NF-001–006, 018             | TC-040(승인 대기 카드), TC-050(클릭 편집), TC-060(승인 속도), TC-070(알림·리마인더) |
...
| API-02 | GET    | `/api/v1/schedules/{id}`                  | 스케줄 ID                                                       | 교대 리스트, 법규 위반 목록, 승인 상태                                  | REQ-FUNC-006–008, 009; REQ-NF-001, 018 |
...
| API-04 | GET    | `/api/v1/reports/payroll?weekOf=YYYY-WW`  | 주차, 선택적 매장 ID                                            | 직원별 급여·수당 상세, 증빙 파일 링크, 고유 해시 ID                     | REQ-FUNC-011–014, 022; REQ-NF-003     |
...
| details        | String     | 변경 내용 요약 (선택 사항)                      | 예: "시급 10000 -> 11000으로 변경"   |
